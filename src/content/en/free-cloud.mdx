...
Although I haven't used DynamoDB, it is similar to MongoDB, which will be mentioned later. It is suitable for storing unstructured data. If you need a NoSQL database, DynamoDB is a great choice, and the free quota offered by AWS looks good, especially the storage part.
...
Like Supabase, MongoDB Atlas also does not limit the number of database requests, which is a blessing for applications that requires writing and reading very often. Additionally, MongoDB offers multiple cloud platforms for database deployment, such as AWS, GCP, Azure, etc., and you can also customize the region where you want to host your database.
