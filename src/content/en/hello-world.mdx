---
slug: "hello-world"
title: "How to print Hello World?"
description: "How to print hello world to console in JavaScript? Read this article if you can't solve this complex coding problem."
image: "/blog/hello-world/og.jpg"
publish: 2024-03-31T17:00:00
modify: 2024-03-31T17:00:00
authors: ["wolf"]
tags: ["javascript"]
---

Sometimes coding is complex, and I'm here to help you out. Today we are going to talk how to print `Hello World!` in console.

To solve this problem, we need to look at this beautiful picture first:

![Rick roll image](/blog/hello-world/rick.webp)

> If you want to ask if this image is relevant to this article, it is.

So how to print `Hello World!` in console? You can try out this simple code snippet:

```ts
console.log("Hello World!")
```

---

You could also print multiple lines of Hello World using `for` loop. We will discuss `for` loop in future blog posts that won't be written at all.

```ts 
for (let i = 0; i < 5; i++) {
  console.log("Hello World!");
}
```

Here is how it works:

1. First, it runs the `for` loop
2. Second, it works.

It should print `Hello World!` for five times, if it didn't, you can try to fix it by:

- Change 
  ```js
  for (let i = 0; i < 5; i++)
  ```
  to
  ```js
  for (let i = 1; i < 6; i++)
  ```
- Or ask ChatGPT at [here](https://w.wolf-yuan.dev/youtube)

