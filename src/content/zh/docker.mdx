...
description: "Docker 的映像檔太大了嗎？多階段建置與 Distroless 基底映像檔可以幫助您減少整體映像檔大小！"
...
> 參考 `donut.c`：https://github.com/limiteci/donut.c/blob/main/donut.c
...
這個最終的映像檔應該比之前的 Debian 或 Alpine 映像檔小很多，因為它基本上只有一個 "donut" 二進位檔而已。
...
以上數據顯示，Distroless + 多階段建置得到了最小的映像檔大小。Alpine 排在第二，而使用多階段的的 Debian 是第三。最後兩個是使用單階段的的 Debian。

雖然在這個比較中，Alpine 為基底的映像檔不是最小的，但考慮到它只有大約 7MB 的大小，它仍然是一個不錯的選擇。

## 結論

所以你應該選擇什麼呢？Distroless 或是 Alpine 或是 Debian 作為基底映像檔？

我的建議是，如果你的應用程式用了一些特別的執行環境，而且恰恰好做那個環境的人有提供已經建立好的 Alpine 映像檔，那麼請使用 Alpine。

但如果你的執行環境已經被 Google 預先建置在 Distroless 裡面了，那麼請使用 Distroless。目前，Distroless 有 `python3`、`java-{17, 21}` 和 `nodejs-{18, 20, 22}`。這些映像檔包含了這些執行環境，而且*應該*是經過優化的。

> [在 GitHub 上面看看有什麼環境已經做好了](http://github.com/GoogleContainerTools/distroless?tab=readme-ov-file#what-images-are-available)
